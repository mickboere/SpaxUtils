...
namespace SpaxUtils
{
	...
	public class AgentCombatComponent : AgentComponentBase
	{
		...
		protected void OnUpdate(float deltaTime)
		{
			...
			Offense =
				Mathf.Max(
					Agent.Stats.GetStat(AgentStatIdentifiers.PIERCING.SubStat(AgentStatIdentifiers.SUB_LEFT_HAND)) ?? 0f,
					Agent.Stats.GetStat(AgentStatIdentifiers.PIERCING.SubStat(AgentStatIdentifiers.SUB_RIGHT_HAND)) ?? 0f);
