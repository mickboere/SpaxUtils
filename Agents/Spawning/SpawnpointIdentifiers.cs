namespace SpaxUtils
{
	public class SpawnpointIdentifiers : ISpawnpointIdentifiers
	{
		private const string GENERIC = ISpawnpointIdentifiers.SPAWNPOINT + "Generic/";
		public const string DEFAULT = GENERIC + "Player";
		public const string TEST_00 = GENERIC + "TEST_00";
		public const string TEST_01 = GENERIC + "TEST_01";
		public const string TEST_02 = GENERIC + "TEST_02";
		public const string TEST_03 = GENERIC + "TEST_03";
		public const string TEST_04 = GENERIC + "TEST_04";
		public const string TEST_05 = GENERIC + "TEST_05";
		public const string TEST_06 = GENERIC + "TEST_06";
		public const string TEST_07 = GENERIC + "TEST_07";
		public const string TEST_08 = GENERIC + "TEST_08";
		public const string TEST_09 = GENERIC + "TEST_09";
	}
}